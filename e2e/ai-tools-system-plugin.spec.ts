...
test("system AI tools consume shared providers and replace live", async ({
  page,
  workspace,
}) => {
  ...
  expect(profile.plugins.find((entry) => entry.id === "ai-tools-system-native"))
    .toMatchObject({
      id: "ai-tools-system-native",
      manifest: {
        schemaVersion: 3,
        id: "ai-tools-system-native",
  ...
  expect(profile.activationOrder).toContain("ai-tools-system-native");
  expect(profile.modules).toContainEqual(
    expect.objectContaining({ pluginId: "ai-tools-system-native" }),
  );

  const result = await page.evaluate(() =>
    window.__termcoE2E.copyAndReplacePluginThroughPlan({
      pluginId: "ai-tools-system-native",
      replacementId: "e2e.ai-tools-system",
    }),
  );
  ...
  const apply = await page.evaluate(() =>
    window.__termco.applyPlugin("e2e.ai-tools-system"),
  );
  ...
  expect(
    replaced.find((entry) => entry.id === "e2e.ai-tools-system")?.name,
  ).toBe("E2E AI Tools: System (Custom)");
});
