...
import type { UiTabDescriptor, UiTabKindContribution, UiTabsRuntime } from "@termco/ui-tabs-base";
...
function PluginErrorCard({ pluginId, message }: { pluginId?: string; message: string }) {
  return (
    <div
    ...
    >
      <div className="text-sm font-medium text-foreground">Plugin render error</div>
      <div className="max-w-sm text-xs text-muted-foreground">
        {pluginId ? (
          <>
            The plugin <code className="rounded bg-accent px-1">{pluginId}</code> crashed while
            rendering this view.
          </>
        ) : (
...
function SurfaceEntry({
  ...
  const isActive = activeTab != null && contribution.kinds.includes(activeTab.kind);
  const searchRegistration = useRef<() => void>(() => {});
  const replaceSearchRegistration = useCallback<ReplaceSearchRegistration>((dispose) => {
  ...
  if ((contribution.mountWhen ?? "always") === "whenOpen" && tabsOfKind.length === 0) {
    return null;
  }
  ...
  return (
    ...
    >
      <ErrorBoundary
        fallback={(error) => <PluginErrorCard pluginId={entry.pluginId} message={error.message} />}
      >
        <Component
...
function PluginTabPlaceholder({ kind }: { kind: string }) {
  return (
    ...
    >
      <div className="text-sm font-medium text-foreground">Plugin not active</div>
      <div className="max-w-sm text-xs text-muted-foreground">
        This tab belongs to a plugin that is currently disabled or not installed ({kind}). Enable
        the plugin to bring the tab back to life — its content is preserved.
      </div>
    </div>
  );
}
